...
class Bar:
    def __init__(self):
        text = Path('/home/alex/Downloads/text-books/tri.txt').read_text(encoding='UTF-8')
        ...
        while True:
            ...
            if not m:
                ...
                word = m.group().lower()
                word = word.replace('_', '')
                if word in STOP_CHARS or word in IGNORED_WORDS or len(word) > 7 or len(word) < 3:
                    prev_word = None
                else:
                    counter[word] += 1
                    if word not in data:
